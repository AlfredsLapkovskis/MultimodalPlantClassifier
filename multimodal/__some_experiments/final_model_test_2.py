...
def main(configuration, neurons):
    builder = DefaultModelBuilder(
        n_classes=N_CLASSES,
        fusion_sizes=neurons,
    )
    ...
    model.fit(
        ...
        callbacks=[
            ...
            keras.callbacks.TensorBoard(
                log_dir=os.path.join(config.get_log_dir(), "final_model_test_2", f"{neurons[0]}x{neurons[1]}x{neurons[2]}_{datetime.now()}"),
                histogram_freq=1,
            ),
        ],
    )
...
if __name__ == "__main__":
    possible_neurons = [64, 128, 256]
    neurons = product(possible_neurons, possible_neurons, possible_neurons)
    neurons = [n for n in neurons if n[0] >= n[1] and n[1] >= n[2]]
